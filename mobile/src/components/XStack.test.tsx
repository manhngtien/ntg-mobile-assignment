...
describe('XStack', () => {
  it('should render children', () => {
    const { getByText } = render(<XStack><Text>Child</Text></XStack>);
    expect(getByText('Child')).toBeTruthy();
  });

  it('should accept gap prop', () => {
    const { getByText } = render(<XStack gap={8}><Text>Gap</Text></XStack>);
    expect(getByText('Gap')).toBeTruthy();
  });

  it('should accept p prop', () => {
    const { getByText } = render(<XStack p={2}><Text>Padding</Text></XStack>);
    expect(getByText('Padding')).toBeTruthy();
  });

  it('should accept style prop', () => {
    const { getByText } = render(<XStack style={{ backgroundColor: 'red' }}><Text>Styled</Text></XStack>);
