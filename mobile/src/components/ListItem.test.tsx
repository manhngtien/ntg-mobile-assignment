...
describe('ListItem', () => {
  it('should render title and body', () => {
    const item = { title: 'Test Title', body: 'Test Body', imageUrl: 'https://example.com/img.jpg' };
    const { getByText } = render(<ListItem item={item} />);
    expect(getByText('Title: Test Title')).toBeTruthy();
    expect(getByText('https://example.com/img.jpg')).toBeTruthy();
  });

  it('should render with empty imageUrl', () => {
    const item = { title: 'No Image', body: 'Body', imageUrl: '' };
    const { getByText } = render(<ListItem item={item} />);
    expect(getByText('Title: No Image')).toBeTruthy();
  });
});
