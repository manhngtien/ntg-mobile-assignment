...
describe('productService', () => {
  ...
  it('should call getProductById with valid id', async () => {
    jest.spyOn(secureStorageService, 'loadToken').mockResolvedValue('my-token');
    const mockProduct = { id: 1, name: 'Product', description: 'Desc', image: 'img.jpg', price: 10, priceUnit: 'USD', createdAt: '2024-01-01', updatedAt: '2024-01-01', category: 'Electronics' };
    const mockResponse = { data: mockProduct };
    mockAxios.get.mockResolvedValue(mockResponse);
    const result = await productService.getProductById(1);
    expect(mockAxios.get).toHaveBeenCalledWith('http://localhost:3000/product/1', { headers: { Authorization: 'Bearer my-token' } });
    expect(result).toEqual(mockProduct);
  });

  it('should call getProductById with correct authorization header', async () => {
    jest.spyOn(secureStorageService, 'loadToken').mockResolvedValue('my-token');
    const mockProduct = { id: 5, name: 'Another', description: '', image: '', price: 20, priceUnit: 'USD', createdAt: '', updatedAt: '', category: '' };
    const mockResponse = { data: mockProduct };
    mockAxios.get.mockResolvedValue(mockResponse);
    await productService.getProductById(5);
    expect(secureStorageService.loadToken).toHaveBeenCalled();
    expect(mockAxios.get).toHaveBeenCalledWith('http://localhost:3000/product/5', { headers: { Authorization: 'Bearer my-token' } });
  });
});
